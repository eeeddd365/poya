...
function renderCalendar() { const grid = document.getElementById('cal-grid'); if(!grid) return; grid.innerHTML = ''; const y = selectedDate.getFullYear(), m = selectedDate.getMonth(), days = new Date(y, m+1, 0).getDate(), dsT = new Date().toISOString().split('T')[0]; for(let d=1; d<=days; d++) { const dS = `${y}-${String(m+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`; const dayData = allMonthData.filter(x => x.date === dS); let html = `<div class="cal-cell ${dS === dsT ? 'cal-is-today' : ''}"><div class="cal-date">${d}</div><div class="staff-tag-group">`; dayData.forEach(x => { if(!["事項","早班值日","晚班值日"].includes(x.staff_name)){ html += `<span>${x.staff_name.substring(0,1)}</span>`; } }); grid.innerHTML += html + `</div></div>`; } }
window.changeDate = (n) => { selectedDate.setDate(selectedDate.getDate()+n); fetchMainData(); };
window.startScanner = (div, inp, cb) => { document.getElementById(div).style.display='block'; html5QrCode = new Html5Qrcode(div); html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: 250 }, (t) => { document.getElementById(inp).value=t; window.stopScan(); if(cb) cb(t); }).catch(()=>alert("相機開啟失敗")); };
...
async function fetchStock(){ const {data}=await _sb.from('stock_items').select('*').eq('status','待處理'); if(data?.some(i=>i.owner_name===currentUser?.name)) alert("你有新的貨物領取通知！"); }
