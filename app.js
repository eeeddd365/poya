...
window.searchInventory = function() {
    ...
    searchTimer = setTimeout(async () => {
        ...
        }).join('') || '<div class="text-center p-4 text-muted small">查無商品</div>';
    }, 300);
};
...
window.uploadToDrive = async function() { 
    const f = document.getElementById('up-drv-file').files[0]; if(!f) return alert("請選擇檔案");
    setLoad(true); 
    const sn = encodeURIComponent(f.name).replace(/%/g, '__');
    const { error } = await _sb.storage.from('public_files').upload(`${Date.now()}_${sn}`, f);
    if(!error) { alert("上傳成功"); fetchDriveFiles(); } else { alert("失敗"); }
    setLoad(false); 
};
...
async function fetchDriveFiles() {
    ...
    l.innerHTML = data?.map(f => {
        ...
        return `<div class="flat-card d-flex justify-content-between align-items-center mb-2"><span class="text-truncate small fw-bold" style="max-width:70%"><i class="far fa-file-pdf text-danger me-2"></i>${d}</span><div><a href="${u}" target="_blank" class="btn btn-sm btn-outline-primary me-2">看</a>${del}</div></div>`;
    }).join('') || '無檔案';
}
...
window.startScan = function() {
    ...
    html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: 250 }, (text) => {
        ...
        window.stopScan(); autoFillInvName(text);
    }).catch(() => alert("相機啟動失敗"));
};
...
async function autoFillInvName(code) { const {data}=await _sb.from('inventory').select('item_name').eq('barcode',code).limit(1); if(data && data.length > 0) document.getElementById('i-name').value = data[0].item_name; }
...
window.submitStock = async function() {
    ...
    try {
        ...
        stM.hide(); fetchStock(); alert("通知已送出");
    } catch(e){ alert("失敗"); } finally { setLoad(false); }
};
