    ...
    try {
        ...
        if (data) {
            ...
            if (currentUser.code === '555') {
                const aEl = document.getElementById('t-adm'); if(aEl) aEl.style.display = 'block';
                const dEl = document.getElementById('drv-admin-area'); if(dEl) dEl.style.display = 'block';
            }
...
window.uploadToDrive = async function() { 
    ...
    if(!error) fetchDriveFiles(); else alert("失敗");
    setLoad(false); 
};
...
// --- 輔助邏輯 ---
function getShortName(name) { const n = String(name||"").trim(); return n.length > 2 ? n.substring(n.length-2) : n; }
