...
window.submitStock = async function() {
    ...
    } catch(e) { alert("投送失敗"); } finally { setLoad(false); }
};
...
function renderCalendar() {
    ...
    const year = selectedDate.getFullYear(), month = selectedDate.getMonth(), daysInMonth = new Date(year, month + 1, 0).getDate();
...
async function fetchStock() {
    ...
    document.getElementById('stk-list').innerHTML = data?.map(i => {
        ...
        return `<div class="flat-card d-flex align-items-center gap-3">${url?`<img src="${url}" style="width:60px;height:60px;object-fit:cover;border-radius:10px;" onclick="window.open('${url}')">`:'<div style="width:60px;height:60px;background:#eee;border-radius:10px;"></div>'}<div class="flex-grow-1"><div class="fw-bold">${i.sender_name} → ${getShortName(i.owner_name)}</div><div class="small text-muted">備註：${i.note}</div><button class="btn btn-sm btn-success w-100 mt-2 rounded-pill" onclick="handleDone('${i.id}','${i.photo_path}')">處理完成</button></div></div>`;
    }).join('') || '<div class="p-4 text-center text-muted">無包裹紀錄</div>';
}

window.handleDone = async function(id, p) { if(!confirm("完成紀錄？")) return; setLoad(true); await _sb.from('stock_items').delete().eq('id', id); if(p && p !== 'null') await _sb.storage.from('photos').remove([p]); fetchStock(); setLoad(false); };
...
async function fetchDriveFiles() {
    ...
    document.getElementById('drv-list').innerHTML = data?.map(f => {
        ...
        return `<div class="flat-card d-flex justify-content-between align-items-center mb-2"><span class="text-truncate small fw-bold" style="max-width:70%">${disp}</span><div><a href="${url}" target="_blank" class="btn btn-sm btn-outline-primary me-2">看</a>${delBtn}</div></div>`;
    }).join('') || '無檔案';
}
...
window.switchTab = function(t) { ['v-ros','v-stk','v-drv','v-adm'].forEach(v=>document.getElementById(v).style.display='none'); ['t-ros','t-stk','t-drv','t-adm'].forEach(v=>document.getElementById(v).classList.remove('active')); document.getElementById('v-'+t).style.display='block'; document.getElementById('t-'+t).classList.add('active'); };
