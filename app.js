...
let stM = null, invM = null, adjM = null, locM = null, html5QrCode = null, curAdjId = null, curAdjQty = 0, curLocBarcode = "", searchTimer = null;
...
window.switchTab = function(t) {
    window.stopScan?.();
    ['v-ros','v-stk','v-inv','v-loc','v-drv','v-adm'].forEach(v => { const el = document.getElementById(v); if(el) el.style.display = 'none'; });
    ['t-ros','t-stk','t-inv','t-loc','t-drv','t-adm'].forEach(tab => { const el = document.getElementById(tab); if(el) el.classList.remove('active'); });
...
window.parseLocCode = function(code) {
    if(!code || String(code).length !== 8 || isNaN(code)) return code;
    const s = String(code);
    const dept = s.substring(0,2), row = parseInt(s.substring(2,4));
    const side = s.substring(4,6) === "01" ? "陽面" : (s.substring(4,6) === "02" ? "陰面" : "不明");
    const seat = parseInt(s.substring(6,8));
    return `${dept}部門 / 第${row}排 / ${side} / 第${seat}座`;
};

window.startLocScan = () => window.startScanner("loc-reader", "loc-search-barcode", false, window.searchLocation);

window.searchLocation = function() {
    clearTimeout(searchTimer);
    searchTimer = setTimeout(async () => {
        const b = document.getElementById('loc-search-barcode').value;
        if(!b) return;
        setLoad(true);
        const { data } = await _sb.from('inventory').select('*').eq('barcode', b).limit(1);
        const area = document.getElementById('loc-result-area');
        if(data && data.length > 0) {
            const item = data[0];
            const display = window.parseLocCode(item.note);
            area.innerHTML = `<div class="flat-card text-center"><div class="fw-bold fs-5">${item.item_name}</div><div class="loc-tag mt-2">貨架位置</div><div class="mt-2 fw-bold fs-4">${display}</div><button class="btn btn-sm btn-outline-primary mt-3 rounded-pill" onclick="window.openLocEdit('${item.barcode}')">修改位置</button></div>`;
        } else {
            area.innerHTML = `<div class="flat-card text-center p-4"><p class="text-muted">查無此商品位置紀錄</p><button class="btn btn-pink rounded-pill" onclick="window.openLocEdit('${b}')">立即建立位置</button></div>`;
        }
        setLoad(false);
    }, 400);
};

window.openLocEdit = function(b) {
    curLocBarcode = b;
    document.getElementById('edit-loc-barcode').innerText = "條碼: "+b;
    document.getElementById('loc-code-input').value = "";
    document.getElementById('loc-preview').innerText = "";
    if(!locM) locM = new bootstrap.Modal(document.getElementById('locModal'));
    locM.show();
    document.getElementById('loc-code-input').oninput = function() {
        if(this.value.length === 8) document.getElementById('loc-preview').innerText = "預覽: "+window.parseLocCode(this.value);
    };
};

window.saveLocation = async function() {
    const code = document.getElementById('loc-code-input').value;
    if(code.length !== 8) return alert("請輸入 8 位代碼");
    setLoad(true);
    const { data } = await _sb.from('inventory').select('id').eq('barcode', curLocBarcode).limit(1);
    if(data && data.length > 0) await _sb.from('inventory').update({ note: code }).eq('barcode', curLocBarcode);
    else {
        const name = prompt("輸入品名", "新商品");
        if(name) await _sb.from('inventory').insert([{ barcode: curLocBarcode, item_name: name, note: code, dept: code.substring(0,2), qty: 0 }]);
    }
...
// --- 倉庫與庫存業務 ---
window.startSearchScan = () => window.startScanner("q-reader", "q-barcode", true, window.searchInventory);
    ...
    searchTimer = setTimeout(async () => {
        ...
        res.innerHTML = data?.map(i => {
            ...
            return `<div class="flat-card d-flex align-items-center gap-3" onclick="window.openAdjust('${i.id}','${i.item_name}',${i.qty},'${u}','${i.note || ''}')">
                ...
                <div class="flex-grow-1"><div class="fw-bold">${i.item_name}</div><div class="small text-muted">條碼: ${i.barcode} | 庫存: <b class="text-danger">${i.qty}</b></div><div class="small text-secondary">備註: ${window.parseLocCode(i.note) || '無'}</div></div><i class="fas fa-edit text-muted"></i></div>`;
...
window.startScanner = function(divId, inputId, autoSearch = false, callback = null) {
    document.getElementById(divId).style.display = 'block';
    html5QrCode = new Html5Qrcode(divId);
    html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: 250 }, (text) => {
        document.getElementById(inputId).value = text;
        window.stopScan();
        if(autoSearch && callback) callback();
    }).catch(() => alert("相機啟動失敗"));
};
