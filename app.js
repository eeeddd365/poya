...
window.openInvModal = function() { if(!invM) invM = new bootstrap.Modal(document.getElementById('invModal')); invM.show(); };
window.openStockModal = async function() { if(!stM) stM = new bootstrap.Modal(document.getElementById('stockModal')); await fetchStaffList(); stM.show(); };
...
window.deleteInventory = async function() {
    if(!curAdjId || !confirm("確定要永久刪除此商品紀錄嗎？")) return;
    setLoad(true);
...
async function fetchMainData() {
    ...
    if(ld)ld.innerHTML=''; if(ln)ln.innerHTML=''; if(nt)nt.innerText="今日無事項"; if(dD)dD.innerText="--"; if(dN)dN.innerText="--";
...
window.submitStock = async function() {
    ...
    await _sb.from('stock_items').insert([{sender_name:currentUser.name, owner_name:o, note:n, photo_path:p, status:'待處理'}]); if(stM)stM.hide(); fetchStock(); alert("通知已送出"); } catch(e){alert("失敗");} finally {setLoad(false);}
};
window.submitInventory = async function() {
    ...
    if(!d||!b||!n||!q) return alert("必填欄位未填"); setLoad(true);
    ...
    await _sb.from('inventory').insert([{dept:d, barcode:b, item_name:n, qty:parseInt(q), note:nt, photo_path:p, creator:currentUser.name}]); if(invM)invM.hide(); alert("庫存已建立"); } catch(e){alert("失敗");} finally {setLoad(false);}
};
...
window.uploadToDrive = async function() { const f = document.getElementById('up-drv-file').files[0]; if(!f) return alert("請選檔案"); setLoad(true); const sn = encodeURIComponent(f.name).replace(/%/g, '__'); const { error } = await _sb.storage.from('public_files').upload(`${Date.now()}_${sn}`, f); if(!error) { alert("上傳成功"); fetchDriveFiles(); } else alert("失敗"); setLoad(false); };
window.deleteFile = async function(n) { if(!confirm("確定刪除檔案？")) return; setLoad(true); await _sb.storage.from('public_files').remove([n]); fetchDriveFiles(); setLoad(false); };
...
window.startScan = function() { document.getElementById('reader').style.display='block'; html5QrCode=new Html5Qrcode("reader"); html5QrCode.start({facingMode:"environment"},{fps:10,qrbox:250},(text)=>{document.getElementById('i-barcode').value=text;window.stopScan();}).catch(()=>alert("相機失敗")); };
